...
---
本项目架构设计和前后端代码开发等均是人工完成，未参与任何 AI 与 Vibe Coding，可放心食用和维护
...
- `项目任务`：多数字员工协作任务，场景类似一个项目中，多次调用不同规则、技能和配置的上述 `Agent Loop` 引擎来依次或并行启动会话任务，并处理它们返回结果的协作问题，即 OPC 模式；`项目任务` 均通过协调器作为入口，规划完成任务
...
- `AgBox` OPC 实现机制的优势为：
    ...
    - 项目中各个数字员工/协调器的任务和消息可关联溯源，层级从项目 -> 任务 -> 消息
